...
function applyMove(z, y, x, role) {
    ...
    // --- NUEVO: Lógica para mostrar las coordenadas en el log ---
    const moveLog = document.getElementById('move-log');
    if (moveLog.innerText === "Esperando movimientos...") moveLog.innerText = "";
    
    // Creamos la entrada del historial
    const entry = document.createElement('div');
    entry.style.borderBottom = "1px solid #334155";
    entry.style.padding = "2px 0";
    entry.innerHTML = `Jugador <strong style="color: ${role === 'X' ? '#0284c7' : '#e11d48'}">${role}</strong>: [Z=${z}, Y=${y}, X=${x}]`;
...
socket.on('restart_game', () => {
    ...
    // --- NUEVO: Limpiamos el log de coordenadas ---
    document.getElementById('move-log').innerHTML = "Esperando movimientos...";
